...
`maps/` 디렉터리에 본인의 `my_map.pgm`, `my_map.yaml` 파일을 복사합니다.
...
# 전역 경로 확인 (RViz2에서 /global_path 추가)
...
- **Pure Pursuit**: lookahead point를 향해 곡률 계산 → 속도 명령
- **VFH (Vector Field Histogram)**: LiDAR를 72개 섹터로 나누어 장애물이 없는 방향(valley) 중 목표 방향에 가장 가까운 방향 선택
...
| 위치추정이 틀림 | RViz2에서 **"2D Pose Estimate"** 로 초기 위치 지정 |
